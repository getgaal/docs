...
## Resource types and guarantees

Every entry in `gaal.yaml` falls into a small set of resource types, and each type has a clear boundary:
...
| **Content** | gaal copies only the configured path mappings. Unsafe relative paths, symlinks, and VCS metadata are skipped. |
...
| **Hooks** | gaal runs only the commands you declare, in order, around a one-shot or service sync. |

The MCP guarantee is the one that matters most: gaal will not clobber your agent config. It reads, modifies the MCP section in place, and writes back.
