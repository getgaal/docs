...
…then the literal string `${GITHUB_TOKEN}` lands in the agent's MCP config. The **agent process** that launches the MCP server is responsible for resolving `${GITHUB_TOKEN}` from the environment.
